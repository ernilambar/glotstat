...
- **JS**: Prettier with the uncustomized `@wordpress/prettier-config`. Vanilla JS, no framework. Global namespace `glotstatData` is injected via `wp_add_inline_script`.
...
All three must pass clean. PHP files must pass PHPCS with no warnings. All files Prettier supports must be formatted; YAML and hidden directories are excluded via `.prettierignore`.
